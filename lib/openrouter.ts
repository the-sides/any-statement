...
For bank statements, return withdrawals, debit-card purchases, checks, outgoing ACH, outgoing wires, fees, interest charges, and other outflows as expense rows. Exclude deposits, incoming transfers, credits, rewards, and balance summary lines. A debit that only moves money between the user's own accounts, or pays a credit card balance, is not an expense: internal transfers and card-account payments duplicate charges that are already itemized on the card statement, so exclude them even though they appear as debits. Debits described as funds transfers (for example "USAA FUNDS TRANSFER DB") or as transfers to or from savings or checking are internal transfers: exclude them even when the description is ambiguous.

Bank statements also carry income. Return every payroll deposit, employer ACH credit, deposited check, interest payment, and peer-to-peer or third-party money received in the incomes array, using the kind field: paycheck for wages, interest for interest paid, other for everything else. Exclude incoming transfers from the user's own other accounts: deposits described as funds transfers between accounts at the same institution (for example "USAA FUNDS TRANSFER CR"), or the matching credit of a withdrawal on another own-account statement, are balance movements, not income, and must never be included with lowered confidence because the label looks ambiguous. Money received from a different person or institution (for example "NFCU ACH P2P VICKI WHITE") is income. Credit card statements have no income, so return an empty incomes array for them.

If a transaction looks like an outflow but you are not fully certain, include it with a lower confidence score instead of omitting it. That rule never applies to internal transfers between the user's own accounts or to card-account payments: those are always excluded, whatever the confidence. The expenses array should be empty only when the statement file contains no transaction detail rows.
...
Bank CSVs also carry income: return payroll deposits, deposited checks, interest payments, and peer-to-peer or third-party money received in the incomes array (kind: paycheck, interest, or other). Exclude incoming transfers from the user's own other accounts: rows described as funds transfers between accounts at the same institution (for example "USAA FUNDS TRANSFER CR") are balance movements, not income, even when the label is ambiguous. Money received from a different person or institution is income. Credit card CSVs have no income, so return an empty incomes array.
